...
public class VlAbyssModKeyMappings {
	...
	public static final KeyMapping HABILIDADE_PRIMARIA = new KeyMapping("key.vl_abyss.habilidade_primaria", GLFW.GLFW_KEY_V, "key.categories.mantra") {
		private boolean isDownOld = false;
	...
	public static final KeyMapping SELECIONA_MANTRA_RAPIDO = new KeyMapping("key.vl_abyss.seleciona_mantra_rapido", GLFW.GLFW_KEY_LEFT_CONTROL, "key.categories.mantra") {
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				VlAbyssMod.PACKET_HANDLER.sendToServer(new SelecionaMantraRapidoMessage(0, 0));
				SelecionaMantraRapidoMessage.pressAction(Minecraft.getInstance().player, 0, 0);
				SELECIONA_MANTRA_RAPIDO_LASTPRESS = System.currentTimeMillis();
			} else if (isDownOld != isDown && !isDown) {
				int dt = (int) (System.currentTimeMillis() - SELECIONA_MANTRA_RAPIDO_LASTPRESS);
				VlAbyssMod.PACKET_HANDLER.sendToServer(new SelecionaMantraRapidoMessage(1, dt));
				SelecionaMantraRapidoMessage.pressAction(Minecraft.getInstance().player, 1, dt);
			}
			isDownOld = isDown;
		}
	};
