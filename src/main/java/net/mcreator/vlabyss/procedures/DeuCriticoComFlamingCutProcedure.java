		...
		if (sourceentity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(VlAbyssModMobEffects.FLAMING_CUT.get())) {
			if ((sourceentity instanceof LivingEntity _livEnt && _livEnt.hasEffect(VlAbyssModMobEffects.FLAMING_CUT.get()) ? _livEnt.getEffect(VlAbyssModMobEffects.FLAMING_CUT.get()).getAmplifier() : 0) == 0) {
				...
				if (world.isClientSide()) {
					SetupAnimationsProcedure.setAnimationClientside((Player) sourceentity, "mantragolpesoco", true);
				}
				if (!world.isClientSide()) {
						...
						synchronized (connections) {
							...
							while (iterator.hasNext()) {
								...
								if (!connection.isConnecting() && connection.isConnected())
									VlAbyssMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.VlAbyssModAnimationMessage(Component.literal("mantragolpesoco"), sourceentity.getId(), true), connection, NetworkDirection.PLAY_TO_CLIENT);
							}
						}
					}
				}
				...
				if (sourceentity instanceof LivingEntity _entity)
					_entity.removeEffect(VlAbyssModMobEffects.FLAMING_CUT.get());
				if (world.isClientSide()) {
					SetupAnimationsProcedure.setAnimationClientside((Player) sourceentity, "mantragolpeespada", true);
				}
				if (!world.isClientSide()) {
					if (sourceentity instanceof Player && world instanceof ServerLevel srvLvl_) {
						List<Connection> connections = srvLvl_.getServer().getConnection().getConnections();
						synchronized (connections) {
							...
							while (iterator.hasNext()) {
								Connection connection = iterator.next();
								if (!connection.isConnecting() && connection.isConnected())
									VlAbyssMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.VlAbyssModAnimationMessage(Component.literal("mantragolpeespada"), sourceentity.getId(), true), connection, NetworkDirection.PLAY_TO_CLIENT);
							}
						}
					}
				}
			...
			} else if ((sourceentity instanceof LivingEntity _livEnt && _livEnt.hasEffect(VlAbyssModMobEffects.FLAMING_CUT.get()) ? _livEnt.getEffect(VlAbyssModMobEffects.FLAMING_CUT.get()).getAmplifier() : 0) >= 2) {
				world.addParticle((SimpleParticleType) (VlAbyssModParticleTypes.FLAMING_SLASH.get()), (entity.getX()), (entity.getY() + 1), (entity.getZ()), 0, 0, 0);
				entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("vl_abyss:mantra_damage")))),
						(float) ((sourceentity instanceof LivingEntity _attributeContext ? _attributeContext.getAttributeValue(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE) : 0.0D)
								- (sourceentity instanceof LivingEntity _attributeContext ? _attributeContext.getAttributeValue(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE) : 0.0D) / 1.4));
				entity.setSecondsOnFire(8);
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(VlAbyssModMobEffects.BURN_SOUL.get(), 200, 0));
				if (sourceentity instanceof LivingEntity _entity)
					_entity.removeEffect(VlAbyssModMobEffects.FLAMING_CUT.get());
				if (world.isClientSide()) {
					SetupAnimationsProcedure.setAnimationClientside((Player) sourceentity, "mantragolpeespada", true);
